...
class FileHandler:
    ...
    def open_editor(self, path):
        ...
        if not self._file_is_writable(file):
            ...
            if admin is not None:
                return admin.launch([self._get_file(f"admin://{file.get_path()}")])

            logging.warning("Could not find a text editor supporting admin://")

            root = self._get_root_editor()
            if root is not None:
                return root.launch([file])

            logging.warning("Could not find a text editor supporting pkexec")

        editor = self._get_editor()
        if editor is not None:
            return editor.launch([file])

        logging.warning("Could not find a supported text editor")
    ...
    def _file_is_writable(self, file: Gio.File):
        try:
            info = file.query_info(
                Gio.FILE_ATTRIBUTE_ACCESS_CAN_WRITE, Gio.FileQueryInfoFlags.NONE, None
            )
    ...
    def _get_root_editor(self):
        ...
        if pkexec_bin is None:
            logging.warning("Could not find pkexec to for executing root editors")
            return None
        if self._get_display_server() == "wayland":
            logging.warning("pkexec is not supported under Wayland")
            return None
        ...
        for appid, editor in self._get_editors().items():
            for pkexec in pkexecs:
                if (
                    pkexec == appid
                    or pkexec == f"org.freedesktop.policykit.pkexec.{appid}"
                ):
    ...
    def _get_display_server(self):
        ...
        if x11 is not None:
            return "x11"
        logging.warning("Could not determine display server. Assuming x11")
        return "x11"
