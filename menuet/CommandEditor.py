...
class CommandEditorDialog(Gtk.Dialog):
    def __init__(self, parent, initial_text, use_header_bar):
        ...
        entry.connect(
            "field-state-changed", self.on_state_changed, "field", fields_hint
        )

        help = HelpButton(
            "https://github.com/schnee-and-tetra/menuet",
            _("Help"),
        )
...
class CommandEditorEntry(Gtk.Box):
    ...
    def __init__(self, initial_text):
        ...
        self.entry.connect(
            "changed", self.on_entry_changed, revert_button, clear_button
        )
    ...
    def validate_field_codes(self, text):
        ...
        for key in keys:
            if quoted[key] > 0:
                return (
                    _("Field code '%s' can not be used inside a quoted argument") % key
                )
    ...
    def validate_cmd(self, text):
        results = {
            ...
            "cmd": None,
            "args": [],
            "errors": {"env": None, "cmd": _("No command found"), "fields": None},
        }
        ...
        for part in parts:
            ...
            if self.is_env_var(part):
                if not env_found:
                    results["errors"]["env"] = _(
                        "Environment variables should be preceded by env"
                    )
                elif non_env_found:
                    results["errors"]["env"] = _(
                        "Environment variables should precede the command"
                    )
                else:
...
class CommandToolBox(Gtk.Box):
    ...
    def __init__(self):
        ...
        label = Gtk.Label.new("")
        label.set_markup("<b>{}</b>".format(_("Add")))
...
class CommandEditorEnvEditor(Gtk.MenuButton):
    ...
    def __init__(self):
        ...
        label = Gtk.Label.new("")
        label.set_markup("<b>{}</b>".format(_("Variable")))
        ...
        label = Gtk.Label.new("")
        label.set_markup("<b>{}</b>".format(_("Value")))
        label.set_xalign(0.00)
        val_entry = Gtk.Entry()
...
class CommandEditorCommandEntry(Gtk.MenuButton):
    ...
    def __init__(self):
        ...
        label = Gtk.Label.new("")
        label.set_xalign(0.00)
        label.set_markup("<b>{}</b>".format(_("Command")))
        entry = Gtk.Entry()
...
class FieldCodeMenu(Gtk.MenuButton):
    __gsignals__ = {"row-activated": (GObject.SignalFlags.RUN_FIRST, None, (str,))}
