...
from .MenuetStackSwitcher import StackSwitcherBox
...
class ApplicationEditor(Gtk.Box):
    ...
    def __init__(self, use_headerbar):
        ...
        self._internal_values = {"Type": "", "Version": ""}
        ...
        label = FieldLabel(_("Working Directory"), "Path", _("The working directory."))
        ...
        label = FieldLabel(
            ...
            "StartupNotify",
            _(
                'If set to "True", a startup notification is sent. Usually means that a busy cursor is shown while the application launches.'
            ),
        )
        label.set_hexpand(True)
        grid.attach(label, 0, 1, 1, 1)
        ...
        label = FieldLabel(
            ...
            "NoDisplay",
            _(
                'If set to "True", this entry will not be shown in menus, but will be available for MIME type associations etc.'
            ),
        )
        label.set_hexpand(True)
        ...
        self._additional_settings.add_child(
            self._category_editor,
    ...
    def set_name_editable(self, editable):
        """Allow or prevent editing the Name field."""
        self._name_entry.set_editable(editable)

    def set_icon_editable(self, editable):
        """Allow or prevent editing the Icon field."""
        self._icon_entry.set_editable(editable)

    def set_comment_editable(self, editable):
        """Allow or prevent editing the Comment field."""
        self._comment_entry.set_editable(editable)
