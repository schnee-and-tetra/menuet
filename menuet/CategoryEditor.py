...
class CategoryEditor(Gtk.Box):
    ...
    def __init__(self):
        ...
        # Flat (non-hierarchical) list of category descriptions, kept in
        # sync with self._category_lookup by _add_category(). self._options
        # is a two-level tree (section -> categories), and whether
        # Gtk.EntryCompletion walks into child rows of a Gtk.TreeStore isn't
        # guaranteed, so completion gets its own flat model instead of
        # reusing self._options directly.
        self._completion_options = Gtk.ListStore.new([str])
        self._completion = Gtk.EntryCompletion()
        self._completion.set_model(self._completion_options)
        ...
        # Allow typing a category directly instead of only picking from the
        # popup list. Combined with the "editing-started" handler below,
        # this also gets autocomplete suggestions as you type.
        ...
        renderer.connect("edited", self._on_combo_edited)
        renderer.connect("editing-started", self._on_combo_editing_started)
        ...
        self._treestore.connect(
            "row-inserted", self._on_row_inserted, treeview, remove_button, clear_button
        )
        self._treestore.connect(
            "row-deleted", self._on_row_deleted, remove_button, clear_button
        )
    ...
    def _on_row_inserted(
        self, model, path, treeiter, treeview, remove_button, clear_button
    ):
    ...
    def _add_category(self, category, section=None):
        ...
        category_sort = f"{section_sort}-{category_desc}"
        self._category_lookup[category] = [category, section_desc, category_desc]
        self._options.append(
            section_iter, self._category_lookup[category] + [category_sort]
        )
        self._completion_options.append([category_desc])
        return self._category_lookup[category]
    ...
    def _on_combo_edited(self, renderer, path, new_text):
        """Commit free-typed (or autocomplete-selected) text.

        GtkCellRendererCombo's "changed" signal only fires when a value is
        picked from the popup list itself -- typing text directly and
        pressing Enter, which is the whole point of has-entry=True and the
        attached EntryCompletion, instead emits the inherited
        GtkCellRendererText "edited" signal. Nothing was connected to it,
        so a typed value never reached self._treestore and the cell just
        reverted to its previous (often empty, for a new row) content.
        """
        new_text = new_text.strip()
        if len(new_text) == 0:
            return
        # If the typed/autocompleted text matches an existing category's
        # description, resolve it back to the same raw category id and
        # section _on_combo_changed would use, so what's saved is the
        # actual XDG category token rather than its translated label.
        for raw_category, section_desc, category_desc in self._category_lookup.values():
            if category_desc == new_text:
                self._treestore[path][COL_CATEGORY] = raw_category
                self._treestore[path][COL_SECTION] = section_desc
                self._treestore[path][COL_DESC] = category_desc
                return
        # Otherwise this is a new, custom category: use the typed text as
        # the raw category token itself, the same as when one is loaded
        # from a launcher that already has a non-standard category.
        raw_category, section_desc, category_desc = self._add_category(new_text)
        self._treestore[path][COL_CATEGORY] = raw_category
        self._treestore[path][COL_SECTION] = section_desc
        self._treestore[path][COL_DESC] = category_desc

    def _on_combo_editing_started(self, renderer, editable, path):
        """Attach autocomplete to the combo's entry each time editing
        starts. GtkCellRendererCombo builds a fresh GtkComboBox for every
        edit session, so the entry (its child, since has-entry=True) has to
        be re-fetched and re-attached each time rather than once in
        __init__.
        """
        entry = editable.get_child()
        if entry is not None:
            entry.set_completion(self._completion)
            entry.connect("focus-out-event", self._on_entry_focus_out, renderer, path)

    def _on_entry_focus_out(self, entry, event, renderer, path):
        self._on_combo_edited(renderer, path, entry.get_text())
        return False
