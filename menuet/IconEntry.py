...
class IconEntry(Gtk.MenuButton):
    ...
    def set_editable(self, editable):
        """Allow or prevent opening the icon picker (Browse Icons/Files)."""
        ...
        if icon_name is not None:
            ...
            # If the icon name is actually a file, render it to the Image.
                ...
                if pixbuf is not None:
                    scaled = pixbuf.scale_simple(48, 48, GdkPixbuf.InterpType.HYPER)
                    self._image.set_from_pixbuf(scaled)
                    self._on_successful_image(icon_name, icon_name)
                    return

            if icon_name.startswith("applications-") or icon_name == "gnome-settings":
...
class IconFileSelectionDialog(Gtk.FileChooserDialog):
    def __init__(self, parent, initial_file="", use_header_bar=False):
        # Temporarily switch to SAVE mode during initial generation to force the input field to appear, then switch to OPEN mode.
        super().__init__(
        ...
        self.activate_action("location-toggle", None)

        self.add_buttons(
            _("Cancel"), Gtk.ResponseType.CANCEL, _("Apply"), Gtk.ResponseType.OK
        )

        # Show all hidden files (such as .local) from the start.
        self.set_property("show-hidden", True)
        ...
        if len(initial_file) > 0:
            ...
            default_dir = os.path.expanduser("~/.local/share/icons")
            if os.path.exists(default_dir):
                self.set_current_folder(default_dir)
