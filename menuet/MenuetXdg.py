...
class MenuetDesktopEntry:
    """Basic class for Desktop Entry files"""

    def __init__(self, filename=None):
        """Initialize the MenuetDesktopEntry instance."""
    ...
    def __setitem__(self, key, value):
        ...
        if key in ["Name", "GenericName", "Comment", "Keywords"]:
            self._set_locale_string("Desktop Entry", key, default_locale, value)

    def load_properties(self, filename):
        """Load the properties."""
        self.keyfile = GLib.KeyFile.new()
        self.keyfile.load_from_file(filename, GLib.KeyFileFlags.KEEP_TRANSLATIONS)
    ...
    def get_named_property(self, group, key, locale_str=None):
        ...
        if (
            key in ["Name", "GenericName", "Comment", "Keywords"]
            and locale_str is not None
        ):
...
def desktop_menu_install(directory_files, desktop_files):
    ...
    for filename in directory_files:
        ...
        vendor_path = relative.replace(f"/{basename}", "")
...
def desktop_menu_uninstall(directory_files, desktop_files):
    ...
    merged_dir = os.path.join(
        GLib.get_user_config_dir(), "menus", "applications-merged"
    )
